...
function AdminGallery() {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      try {
        const res = await fetch("http://127.0.0.1:8000/auth/check-admin", {
          headers: {
        ...
        if (!res.ok) {
          ...
          fetchPosts(); // загружаем посты после проверки
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editId
        ? `http://127.0.0.1:8000/posts/${editId}`
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <form
        ...
        className="space-y-4 bg-white shadow-md p-4 rounded-md"
      >
        <h2 className="text-xl font-bold">
          {editId ? "Редактировать пост" : "Добавить пост"}
        </h2>
        <input
          type="text"
